#!/usr/bin/env bash
#
# Launch the dual-head pProp MLP wandb Bayesian sweep on SLURM, packing several
# agents onto GPUs via NVIDIA MPS (each task gets a slice of one GPU).
#
...
SWEEP_ID="${SWEEP_ID:-j6z4dh1u}"   # <-- replace with ID from `wandb sweep`
